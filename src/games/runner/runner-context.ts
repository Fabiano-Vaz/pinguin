...
import {
  getPenguinPet,
  getPenguinRunnerModules,
  setPenguinRunnerGame,
...
const snowmanObstacleImage =
  assetsFactory.snowmanObstacleImage || "assets/snowman.svg";
...
const RUNNER_PENGUIN_VISUAL_OFFSET_Y =
  runnerConfig.penguinVisualOffsetYPx || 10;
...
const centerPenguin = () => {
  game.penguin.x = Math.round(
    window.innerWidth * (runnerConfig.penguinCenterXRatio || 0.23),
  );
...
const playEnterTransition = () => {
  ...
  requestAnimationFrame(() => {
    ...
    transitionOverlay.style.opacity = String(
      runnerConfig.transitionOverlayMaxOpacity || 0.55,
    );
    setTimeout(() => {
...
const createGroundDecor = () => {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const x = Math.round(
      (i / count) * width +
        Math.random() * (runnerConfig.groundDecorXJitterPx || 40),
    );
...
const difficultyLevel = () => {
  ...
  const byTime =
    game.worldTimeMs / (runnerConfig.difficultyTimeDivisorMs || 16000);
