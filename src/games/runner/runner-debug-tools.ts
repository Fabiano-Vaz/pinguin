(() => {
  ...
  modules.createDebugTools = ({ DEBUG, game, runnerConfig, debugCollisionDot, debugHitboxLayer }) => {
    ...
    const showDebugCollisionDot = (a, b) => {
      ...
      const centerX = hasOverlap ? overlapLeft + (overlapRight - overlapLeft) / 2 : b.x + b.width / 2;
      const centerY = hasOverlap ? overlapTop + (overlapBottom - overlapTop) / 2 : b.y + b.height / 2;
