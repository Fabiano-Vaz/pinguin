...
modules.penguinRuntimeState = ({ actionStates, runtime }) => ({
  setActivityMode(nextMode, reason = "", { force = false } = {}) {
    if (
      !this.stateService ||
      typeof this.stateService.setActivityMode !== "function"
    ) {
      return false;
    }
    const changed = this.stateService.setActivityMode(nextMode, reason, {
      force,
    });
  ...
  inferActivityMode() {
    ...
    if (this.isFishingActive || this.currentState === "fishing")
      return "fishing";
  ...
  setManagedInterval(context, key, callback, delayMs = 0) {
    ...
    return this.stateService.setManagedInterval(
      context,
      key,
      callback,
      delayMs,
    ...
    if (!this.debugPanelComponent) {
      this.debugPanelComponent = new DebugPanelComponent({
        onAction: (action) => {
          ...
          if (
            action === "next-action" &&
            typeof this.debugAdvanceAction === "function"
          ) {
  ...
  hideDebugPanel() {
    if (
      this.debugPanelComponent &&
      typeof this.debugPanelComponent.dispose === "function"
    ) {
  ...
  renderDebugPanel(now = performance.now()) {
    ...
    const formatDebugStep = (step) => {
      ...
      if (type === "act") {
        ...
        const duration = Number.isFinite(step.duration)
          ? ` ${step.duration}ms`
          : "";
    ...
    const tx = Math.round(
      Number.isFinite(this.targetX) ? this.targetX : this.x,
    );
    const ty = Math.round(
      Number.isFinite(this.targetY) ? this.targetY : this.y,
    );
    ...
    const queueEmptyText =
      Number.isFinite(this.nextBehaviorDueAt) &&
      this.nextBehaviorDueAt > Date.now()
    ...
    this.debugPanelComponent.render({
      ...
      pendingFoodText: Array.isArray(this.foodTargets)
        ? this.foodTargets.length
        : 0,
  ...
  debugAdvanceActivity() {
    ...
    if (typeof this.setActivityMode === "function") {
      this.setActivityMode(nextActivity, "debug:advance-activity", {
        force: true,
      });
    } else {
  ...
  debugAdvanceState() {
    ...
    const cycleStates = Object.keys(actionStates || {}).filter((state) => {
      ...
      return (
        typeof actionStates[state] === "string" &&
        actionStates[state].length > 0
      );
    });
    if (cycleStates.length === 0) return false;

    this.debugStateCycleIndex =
      (Number.isFinite(this.debugStateCycleIndex)
        ? this.debugStateCycleIndex
        : -1) + 1;
    ...
    const stateAsset =
      actionStates[nextState] || actionStates.idle || actionStates.default;
