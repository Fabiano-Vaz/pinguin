...
modules.penguinRuntimeUpdate = ({ runtime, halfPenguinSize, penguinSize }) => ({
  update(now = performance.now()) {
    ...
    try {
      ...
      if (this.isTemporaryDead) {
        ...
        this.targetY = this.y;
        this.element.style.left = this.x + "px";
        // Pull the element down so the lying sprite appears below the ground line.
        // scale(1.45) with transform-origin:center shifts the visual top up by
        // ~0.225 * penguinSize; adding 0.5 * penguinSize pushes it clearly below.
        this.element.style.top = this.y + Math.round(penguinSize * 0.3) + "px";
        this.applyTransform();
        this.updateBubblePosition();
        this.updateUmbrellaPosition();
        this.renderDebugPanel(now);
      ...
      if (typeof this.updateBubblePosition === "function")
        this.updateBubblePosition();
      if (typeof this.updateUmbrellaPosition === "function")
        this.updateUmbrellaPosition();
      this.renderDebugPanel(now);
      requestAnimationFrame((ts) => this.update(ts));
    }
  ...
  blowAwayUmbrella(direction = 1) {
    ...
    const travelY = Math.round(
      -(window.innerHeight * (0.24 + Math.random() * 0.14)),
    );
