const createEnvironmentEvents = (deps) => {
  ...
  const onClick = (event) => {
    ...
    // Checks against clicking on the penguin element itself — kept for all paths
    const clickTarget =
    ...
    if (
      clickTarget &&
      (clickTarget.closest(".penguin") ||
        clickTarget.closest(".penguin-umbrella"))
    ) {
      return;
    }
    const eventPath =
    ...
    // Weather effects (lightning / block) must be checked before any penguin-state
    // guards so that clicking during rain always produces a flash regardless of
    // whether the penguin is in caveirinha/fishing/dragging mode, etc.
    const isRaining =
